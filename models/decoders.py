...
class BaselineDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        recons, masks = x_all.reshape(1, -1, x_all.shape[1], x_all.shape[2], x_all.shape[3]).split([3, 1], dim=-1)
...
class SpatialBroadcastDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)
        recons, masks = x.reshape(bs, -1, x.shape[1], x.shape[2], x.shape[3]).split([3, 1], dim=-1)
        masks = nn.Softmax(dim=1)(masks)
        xhs = recons * masks
