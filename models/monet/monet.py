...
from models.monet.base_model import BaseModel
...
class Monet(BaseModel):
    ...
    encoder_model: EncoderNet = field(init=False)
    decoder_model: BroadcastDecoderNet = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.encoder_model = EncoderNet(**self.encoder_params)
        self.decoder_model = BroadcastDecoderNet(**self.decoder_params)
    ...
    def decoder(self, zh):
        zh = zh.view(1, self.num_slots, self.latent_size)
        slots_recon, masks_pred = self._decode(zh)
        recons = (slots_recon * masks_pred.unsqueeze(2)).sum(dim=1)
        return recons
    ...
    def _encode(self, x: Tensor, log_masks: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        mean, log_sigma = self.encoder_model(encoder_input).chunk(2, dim=1)
        sigma = log_sigma.exp()
    ...
    def _decode(self, zs: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        decoder_output = self.decoder_model(zs)
