...
class MatplotlibUI(object):
    ...
    def __init__(self):
        ...
        plt.ion()
        self._fig = plt.figure(figsize=(9, 12), num="Spriteworld", facecolor="white")
...
class HumanDragAndDropAgent(object):
    ...
    def help(self):
        logging.info(
            "Click to select an object, then click again to select where " "to move it."
        )
    ...
    def step(self, timestep):
        ...
        def _get_click():
            ...
            while click is None:
                ...
                if x is None:
                    logging.info(
                        "Timed out. You took longer than %d seconds to click.",
                        self._timeout,
                    )
                elif x:
                    logging.info(
                        "You pressed a key, but were supposed to click with the "
                        "mouse."
                    )
                    self.help()
                else:
        ...
        def _wait_for_movement_key_press():
            ...
            while not ready:
                ...
                if x is None:
                    ...
                    logging.info(
                        "You clicked, but you are supposed to use the Keyboard."
                    )
                    self.help()
...
def setup_run_ui(env_config, render_size, task_hsv_colors, anti_aliasing):
    ...
    env_config["renderers"] = {
        "image": renderers.PILRenderer(
            ...
            color_to_rgb=renderers.color_maps.hsv_to_rgb if task_hsv_colors else None,
            anti_aliasing=anti_aliasing,
        ),
        "success": renderers.Success(),
    }
